...
class tmon_system:
    ...
    def H_I(self, delta_arr = None):
        ...
        for n in range(self.N):
            for i in range(self.d + 1):
                c_ni = (2*np.pi)*((self.g_arr[n][i-1]**2)*np.heaviside(i,0)/(self.v_arr[n][i-1]- self.omega) + self.v_arr[n][i] - delta_arr[i])
        ...
        for n in range(self.N):
            for m in range(self.N):
                ...
                #correction factor of sqrt(i+1)sqrt(j+1) = (i+1) in c_nmi
                    if n == m:
                        break
                    c_nmi = (2*np.pi)*0.5*(self.g_arr[n][i]*self.g_arr[m][i]*(i+1))/(self.omega - (self.v_arr[m][i+1]-self.v_arr[m][i]))
                    print(self.omega , self.v_arr[m][i],c_nmi)
...
# def generate_tmon_arch(N,Nlevel, identical = True, g = 1, o = 59.5):
    #scaling is 1 to 100MHz
    if identical == True:
        E_j =  np.absolute(np.random.normal(165.375,0, N))
        E_c = np.absolute(np.random.normal(3.0,0, N))    
    else:
        E_j =  np.absolute(np.random.normal(165.375,20, N))
        E_c = np.absolute(np.random.normal(3.0,0.2, N))
